...
function checkSerializeTupleVisitor(
    code: string,
    expected: string,
    warn = false,
    error = false,
): void {
    const visitor = new SerializeTupleVisitor(newProgram(newOptions()), null);
...
`.trim();
        checkSerializeTupleVisitor(code, expected);
    });
...
`.trim();
        checkSerializeTupleVisitor(code, "", false, true);
    });
});
